...
SELECT MARK,COLOR,COUNT(MARK)
FROM auto
GROUP BY MARK,COLOR
HAVING MARK IN ("LADA","BMW");

--  Вывести на экран марку авто и количество AUTO не этой марки
SELECT mark, 
       (SELECT COUNT(*) 
        FROM auto AS auto2 
        WHERE auto2.mark <> auto1.mark) AS count_other_brands
FROM auto AS auto1
GROUP BY mark;

CREATE TABLE test_a (id_a INT, data VARCHAR(10));
create table test_b (id_b INT);
insert into test_a(id_a, data) values
(10, 'A'),
(20, 'A'),
(30, 'F'),
(40, 'D'),
(50, 'C');
insert into test_b(id_b) values
(10),
(30),
(50);
SELECT *
...
FROM test_a
LEFT JOIN test_b
ON test_a.id_a=test_b.id_b
WHERE test_b.id_b is NULL;